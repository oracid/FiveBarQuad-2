// Definitive version of FiveBarQuad - 11/09/2019
// This program uses my Inverse Kinematics function to move the FiveBarQuad robot
#include <Servo.h>
...
int Speed = 4;                                              // to decrease the speed, increase the value

//Rectangle movement step 5 mm with 15 to 25 gap - 29/08/2019 - Fonctionne bien, grande souplesse
...
int FRy[]= { 15, 15, 15, 15, 15, 15, 15, 15, 15,     30, 30, 30, 30, 30, 30, 30, 30, 30};
int BRy[]= { 30, 30, 30, 30, 30, 30, 30, 30, 30,     15, 15, 15, 15, 15, 15, 15, 15, 15};
int FLy[]= { 30, 30, 30, 30, 30, 30, 30, 30, 30,     15, 15, 15, 15, 15, 15, 15, 15, 15};
int BLy[]= { 15, 15, 15, 15, 15, 15, 15, 15, 15,     30, 30, 30, 30, 30, 30, 30, 30, 30};
...
void  loop(){
  ...
  if (distance_F <= 60){
    if (distance_L < distance_R)  {for (int i=0;i<8;i++){TurnRight();}}
    else                          {for (int i=0;i<8;i++){TurnLeft();}}
  }
  
  if (distance_L <= 30) {if (distance_L < distance_R){for (int i=0;i<4;i++){TurnRight();}}}
  if (distance_R <= 30) {if (distance_R < distance_L){for (int i=0;i<4;i++){TurnLeft();}}}
...
void InverseKinematics(int Px, int Py, Servo srvL, Servo srvR, int srvLon, int srvRon){       // below, 12 lines to calculate the angle of the servos
  ...
  float A1x=-16, A1y=160, A2x=A1x+g, A2y=A1y;                   //  2- Values of the servos positions
  float a1=128, c1=32, a2=a1, c2=c1;                            //  3- Values of the length of the leg's bars
